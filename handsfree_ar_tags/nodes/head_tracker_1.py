...
class HeadTracker():
	...
	self.pitch_joint = 'pitch_joint'
        self.tilt_topic = '/mobile_base/pitch_position_controller/command'
	self.pan_topic = '/mobile_base/yaw_position_controller/command'
        ...
        self.max_pan = rospy.get_param('~max_pan', 1.2)
        self.min_pan = rospy.get_param('~min_pan', -1.2)
        self.max_tilt = rospy.get_param('~max_tilt', 0.9)
        self.min_tilt = rospy.get_param('~min_tilt', -0.9)
        ...
        rospy.Subscriber('/mobile_base/joint_states', JointState, self.update_joint_state)
	...
	rospy.Subscriber('ar_pose_marker', AlvarMarkers, self.update_joint_positions)
	...
	self.servo_position[self.head_tilt_joint] = rospy.Publisher(self.tilt_topic, Float64, queue_size=5)
	self.servo_position[self.head_pan_joint] = rospy.Publisher(self.pan_topic, Float64, queue_size=5)
    ...
    def update_joint_state(self, msg):
        try:
            test = msg.name.index(self.pitch_joint)
